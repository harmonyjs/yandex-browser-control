...
const fileDestination: DestinationStream = destination({ dest: logFilePath, append: true, sync: false });
...
export const logger = pino(options, multistream([
...
export const flushLogs = async (): Promise<void> => {
  ...
  try {
    if (isFlushable(fileDestination)) {
      ...
      else if (typeof fileDestination.flushSync === 'function') fileDestination.flushSync();
    }
  } catch {
